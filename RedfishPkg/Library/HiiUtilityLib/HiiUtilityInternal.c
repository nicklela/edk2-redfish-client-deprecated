  ...
  )
{
  ...
  return BufferToValue (Question, Value, &Question->Value);
}
...
ReGetDefault:
  ...
  if ((Action > 0) && ((Question->QuestionFlags & EFI_IFR_FLAG_CALLBACK) != 0) && (ConfigAccess != NULL)) {
    ...
    if (!EFI_ERROR (Status)) {
      if (HiiValue->Type == EFI_IFR_TYPE_STRING) {
        NewString = GetToken (Question->Value.Value.string, FormSet->HiiHandle);
        ASSERT (NewString != NULL);

        ASSERT (StrLen (NewString) * sizeof (CHAR16) <= Question->StorageWidth);
        if (StrLen (NewString) * sizeof (CHAR16) <= Question->StorageWidth) {
          ZeroMem (Question->Value.Buffer, Question->StorageWidth);
          CopyMem (Question->Value.Buffer, NewString, StrSize (NewString));
        } else {
          CopyMem (Question->Value.Buffer, NewString, Question->StorageWidth);
        }
      ...
      if (Default->DefaultId == DefaultId) {
        if (Default->ValueExpression != NULL) {
          ...
          if (Default->ValueExpression->Result.Type == EFI_IFR_TYPE_BUFFER) {
            ASSERT (HiiValue->Type == EFI_IFR_TYPE_BUFFER && Question->Value.Buffer != NULL);
            if (Question->StorageWidth > Default->ValueExpression->Result.BufferLen) {
              CopyMem (Question->Value.Buffer, Default->ValueExpression->Result.Buffer, Default->ValueExpression->Result.BufferLen);
              Question->Value.BufferLen = Default->ValueExpression->Result.BufferLen;
            } else {
              CopyMem (Question->Value.Buffer, Default->ValueExpression->Result.Buffer, Question->StorageWidth);
              Question->Value.BufferLen = Question->StorageWidth;
            }
        ...
        if (HiiValue->Type == EFI_IFR_TYPE_STRING) {
          ...
          if (Question->StorageWidth > StrSize (StrValue)) {
            ZeroMem (Question->Value.Buffer, Question->StorageWidth);
            CopyMem (Question->Value.Buffer, StrValue, StrSize (StrValue));
          } else {
            CopyMem (Question->Value.Buffer, StrValue, Question->StorageWidth);
          }
        }
  ...
  case EFI_IFR_ORDERED_LIST_OP:
    ...
    while (!IsNull (&Question->OptionListHead, Link)) {
      ...
      SetArrayData (Question->Value.Buffer, Question->Value.Type, Index, Option->Value.Value.u64);
